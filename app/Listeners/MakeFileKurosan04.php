...
class MakeFileKurosan04
{
    ...
    public function handle(CheckSignalKurosan04 $event)
    {
        ...
        */
        ...
        if (\File::exists($filepath_signal .'/'. $today .'_kurosan03.txt')) {
            var_dump($filepath_signal .'/'. $today .'_kurosan03.txt');
            $contents_compare = \File::get($filepath_signal .'/'. $today .'_kurosan03.txt');  //比較ファイル
                    ...
                    if (intval($dailysArray[1]) < intval($dailysArray[3])) {  //終値 < 始値 ?
                        ...
                        if ($compare_startpos) {
                            ...
                            if (intval($dailysArray[1]) < intval($compare_endvalue03)) {
                                ...
                                //ファイル出力
                                Storage::append(('kabus/signal/'. $today .'_kurosan.txt'), $fileOutputString);
                            }
                        }
                    }
                }
        ...
        if (!(\File::exists($filepath_signal .'/'. $today .'_kurosan.txt'))) {
            $fileOutputString = ('---' .'/'. '---' .'/'. '---' .'/'. '---' .'/'. '---' .'\n');
            //ファイル出力
            Storage::append(('kabus/signal/'. $today .'_kurosan.txt'), $fileOutputString);
        }
