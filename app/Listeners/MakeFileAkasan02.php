...
use App\Events\CheckSignalAkasan02;
...
class MakeFileAkasan02
{
     ...
     * Handle the event.
     *
     * @param  CheckSignalAkasan02  $event
     * @return void
     */
    public function handle(CheckSignalAkasan02 $event)
    {
        $dt = Carbon::now();
        //基準日(当日)の算出
        //時間チェック
        ...
        if (\File::exists($filepath_signal .'/'. $today .'_akasan01.txt')) {
            var_dump($filepath_signal .'/'. $today .'_akasan01.txt');
            $contents_compare = \File::get($filepath_signal .'/'. $today .'_akasan01.txt');  //比較ファイル
                    ...
                    if (intval($dailysArray[1]) > intval($dailysArray[3])) {  //終値 > 始値 ?
                        ...
                        if ($compare_startpos) {
                            ...
                            if (intval($dailysArray[1]) > intval($compare_endvalue)) {
                                ...
                                //ファイル出力
                                Storage::append(('kabus/signal/'. $today .'_akasan02.txt'), $fileOutputString);
                            }
                        }
                    }
                }
