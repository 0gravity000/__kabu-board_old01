...
use App\Events\CheckSignalAkasan04;
...
class MakeFileAkasan04
{
     ...
     * Handle the event.
     *
     * @param  CheckSignalAkasan04  $event
     * @return void
     */
    public function handle(CheckSignalAkasan04 $event)
    {
        $dt = Carbon::now();
        /*
        //基準日(当日)の算出
        ...
        */
        ...
        if (\File::exists($filepath_signal .'/'. $today .'_akasan03.txt')) {
            var_dump($filepath_signal .'/'. $today .'_akasan03.txt');
            $contents_compare = \File::get($filepath_signal .'/'. $today .'_akasan03.txt');  //比較ファイル
                ...
                if ($dailysArray[1] != "---") { //終値が---でないもの
                    if (intval($dailysArray[1]) > intval($dailysArray[3])) {  //終値 > 始値 ?
                        ...
                        if ($compare_startpos) {
                            ...
                            $compare_endvalue03 = mb_substr($contents_compare, $compare_startpos, $compare_endpos - $compare_startpos);
                            var_dump($dailysArray[1] .':'. $compare_endvalue01 .':'. $compare_endvalue02.':'. $compare_endvalue03);

                            //var_dump($compare_endvalue);
                            //終値 > 前日終値 ?
                            if (intval($dailysArray[1]) > intval($compare_endvalue03)) {
                                //終値 < 前日終値 ?
                                $fileOutputString = ('C'. $code .'/'. $compare_endvalue01 .'/'. $compare_endvalue02 .'/'. $compare_endvalue03 .'/'. $dailysArray[1] .'\n');
                                //ファイル出力
                                Storage::append(('kabus/signal/'. $today .'_akasan.txt'), $fileOutputString);
                            }
                        }
                    }
                }
        ...
        if (!(\File::exists($filepath_signal .'/'. $today .'_akasan.txt'))) {
            $fileOutputString = ('---' .'/'. '---' .'/'. '---' .'/'. '---' .'/'. '---' .'\n');
            //ファイル出力
            Storage::append(('kabus/signal/'. $today .'_akasan.txt'), $fileOutputString);
        }
