...
use App\Events\CheckSignalAkasan01;
...
class MakeFileAkasan01
{
     ...
     * Handle the event.
     *
     * @param  CheckSignalAkasan01  $event
     * @return void
     */
    public function handle(CheckSignalAkasan01 $event)
    {
        $dt = Carbon::now();
        /*
        //基準日(当日)の算出
            ...
            while(mb_strpos($contents, '\n', $startpos)){
                ...
                if ($dailysArray[1] != "---") { //終値が---でないもの
                    if (intval($dailysArray[1]) > intval($dailysArray[3])) {  //終値 > 始値 ?
                        ...
                        Storage::append(('kabus/signal/'. $today .'_akasan01.txt'), $fileOutputString );
                    }
                }
                //var_dump($fileloop);
                $startpos = $endpos+1;
